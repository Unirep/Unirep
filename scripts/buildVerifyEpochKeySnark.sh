...
# NODE_OPTIONS=--max-old-space-size=8192 node build/buildSnarks.js -i circuits/test/verifyEpochKey_test.circom -j build/verifyEpochKeyCircuit.r1cs -w build/verifyEpochKey.wasm -p build/verifyEpochKeyPk.json -v build/verifyEpochKeyVk.json -s build/EpochKeyValidityVerifier.sol -vs EpochKeyValidityVerifier -z build/verifyEpochKey.zkey
