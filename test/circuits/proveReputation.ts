...
describe('Prove reputation from attester circuit', function () {
    ...
    it('successfully prove a random generated reputation', async () => {
        ...
        const results = await genProofAndPublicSignals(Circuit.proveReputation,stringifyBigInts(circuitInputs))
    ...
    it('prove reputation nullifiers with insufficient rep score', async () => {
        ...
        let results = await genProofAndPublicSignals(Circuit.proveReputation,stringifyBigInts(circuitInputs1))
        let endTime = new Date().getTime()
        console.log(`Gen Proof time: ${endTime - startTime} ms (${Math.floor((endTime - startTime) / 1000)} s)`)
        let isValid = await verifyProof(Circuit.proveReputation,results['proof'], results['publicSignals'])
        expect(isValid).to.be.false
        ...
        results = await genProofAndPublicSignals(Circuit.proveReputation,stringifyBigInts(circuitInputs2))
        endTime = new Date().getTime()
        console.log(`Gen Proof time: ${endTime - startTime} ms (${Math.floor((endTime - startTime) / 1000)} s)`)
        isValid = await verifyProof(Circuit.proveReputation,results['proof'], results['publicSignals'])
        expect(isValid).to.be.false
        ...
        startTime = new Date().getTime()
        results = await genProofAndPublicSignals(Circuit.proveReputation,stringifyBigInts(circuitInputs3))
        endTime = new Date().getTime()
        console.log(`Gen Proof time: ${endTime - startTime} ms (${Math.floor((endTime - startTime) / 1000)} s)`)
        isValid = await verifyProof(Circuit.proveReputation,results['proof'], results['publicSignals'])
        expect(isValid).to.be.true
