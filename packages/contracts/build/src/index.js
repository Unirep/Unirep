...
exports.UnirepFactory =
    exports.getUnirepContract =
    exports.deployUnirep =
    exports.computeProcessAttestationsProofHash =
    exports.computeStartTransitionProofHash =
    exports.UserTransitionProof =
    exports.SignUpProof =
    exports.ReputationProof =
    exports.EpochKeyProof =
    exports.Attestation =
    exports.AttestationEvent =
    exports.Event =
        void 0
...
Object.defineProperty(exports, 'UnirepFactory', {
    enumerable: true,
    get: function () {
        return typechain_1.Unirep__factory
    },
})
var Event
;(function (Event) {
...
;(function (AttestationEvent) {
    AttestationEvent[(AttestationEvent['SendAttestation'] = 0)] =
        'SendAttestation'
    AttestationEvent[(AttestationEvent['Airdrop'] = 1)] = 'Airdrop'
    AttestationEvent[(AttestationEvent['SpendReputation'] = 2)] =
        'SpendReputation'
...
class EpochKeyProof {
    constructor(_publicSignals, _proof) {
        this.verify = () => {
            const proof_ = (0, circuits_1.formatProofForSnarkjsVerification)(
                this.proof.map((n) => n.toString())
            )
            return (0, circuits_1.verifyProof)(
        ...
        this.hash = () => {
            const iface = new ethers_1.ethers.utils.Interface(
                typechain_1.Unirep__factory.abi
            )
        ...
        const formattedProof = (0, circuits_1.formatProofForVerifierContract)(
            _proof
        )
    ...
    constructor(_publicSignals, _proof) {
        ...
        this.hash = () => {
            ...
            const abiEncoder = ethers_1.ethers.utils.defaultAbiCoder.encode(
                [
                    `tuple(uint256[${config_1.maxReputationBudget}] repNullifiers,
                    uint256 epoch,
                    uint256 epochKey, 
        ...
        this.proveReputationAmount =
            _publicSignals[config_1.maxReputationBudget + 4]
...
class SignUpProof {
    constructor(_publicSignals, _proof) {
        ...
        this.hash = () => {
            ...
            const abiEncoder = iface.encodeFunctionData('hashSignUpProof', [
                this,
            ])
            return ethers_1.ethers.utils.keccak256(rmFuncSigHash(abiEncoder))
        }
...
class UserTransitionProof {
    constructor(_publicSignals, _proof) {
        ...
        this.transitionFromEpoch =
            _publicSignals[1 + config_1.numEpochKeyNoncePerEpoch]
        this.blindedUserStates.push(
            _publicSignals[2 + config_1.numEpochKeyNoncePerEpoch]
        )
        this.blindedUserStates.push(
            _publicSignals[3 + config_1.numEpochKeyNoncePerEpoch]
        )
        this.fromGlobalStateTree =
            _publicSignals[4 + config_1.numEpochKeyNoncePerEpoch]
        for (let i = 0; i < config_1.numEpochKeyNoncePerEpoch; i++) {
            this.blindedHashChains.push(
                _publicSignals[5 + config_1.numEpochKeyNoncePerEpoch + i]
            )
        }
        this.fromEpochTree =
            _publicSignals[5 + config_1.numEpochKeyNoncePerEpoch * 2]
        this.proof = formattedProof
        this.publicSignals = _publicSignals
    }
}
...
const deployUnirep = async (deployer, _treeDepths, _settings) => {
    ...
    EpochKeyValidityVerifierContract =
        await new typechain_1.EpochKeyValidityVerifier__factory(
            deployer
        ).deploy()
    ...
    StartTransitionVerifierContract =
        await new typechain_1.StartTransitionVerifier__factory(
            deployer
        ).deploy()
    ...
    ProcessAttestationsVerifierContract =
        await new typechain_1.ProcessAttestationsVerifier__factory(
            deployer
        ).deploy()
    ...
    UserStateTransitionVerifierContract =
        await new typechain_1.UserStateTransitionVerifier__factory(
            deployer
        ).deploy()
    await UserStateTransitionVerifierContract.deployTransaction.wait()
    console.log('Deploying ReputationVerifier')
    ReputationVerifierContract =
        await new typechain_1.ReputationVerifier__factory(deployer).deploy()
    await ReputationVerifierContract.deployTransaction.wait()
    console.log('Deploying UserSignUpVerifier')
    UserSignUpVerifierContract =
        await new typechain_1.UserSignUpVerifier__factory(deployer).deploy()
    ...
    const c = await new typechain_1.Unirep__factory(deployer).deploy(
        _treeDepths,
        {
            maxUsers: _maxUsers,
    ...
    console.log(
        ...
        Math.floor(typechain_1.Unirep__factory.bytecode.length / 2),
        'bytes'
    )
