import { keccak256, getCreate2Address, ContractFactory, Signer, TransactionResponse } from "ethers"

export const globalDeployerAddress = '0x7A0D94F55792C434d74a40883C6ed8545E406D12'
...
const GlobalFactory = async (factory: ContractFactory, _signer: Signer) => {
    const signer = _signer ?? factory.runner
        ...
        if (globalDeployerCode === '0x') {
            ...
            // need to deploy
            await signer.provider?.broadcastTransaction(deployTx).then(t => t.wait())
        }
