    ...
    describe('User state transition events with no attestation', async () => {
        ...
        it('Users should successfully perform user state transition', async () => {
            for (let i = 0; i < userIds.length; i++) {
                ...
                let hashedProof = (0, contracts_1.computeStartTransitionProofHash)(startTransitionProof.blindedUserState, startTransitionProof.blindedHashChain, startTransitionProof.globalStateTreeRoot, (0, circuits_1.formatProofForVerifierContract)(startTransitionProof.proof));
                ...
                for (let i = 0; i < processAttestationProofs.length; i++) {
                    ...
                    (0, chai_1.expect)(receipt.status).to.equal(1);
                    let hashedProof = (0, contracts_1.computeProcessAttestationsProofHash)(processAttestationProofs[i].outputBlindedUserState, processAttestationProofs[i].outputBlindedHashChain, processAttestationProofs[i].inputBlindedUserState, (0, circuits_1.formatProofForVerifierContract)(processAttestationProofs[i].proof));
                    proofIndexes.push(Number(await unirepContract.getProofIndex(hashedProof)));
                }
        ...
        it('Submit invalid start tranistion proof should not affect Unirep State', async () => {
            ...
            let hashedProof = (0, contracts_1.computeStartTransitionProofHash)(randomBlindedUserState, randomBlindedHashChain, randomGSTRoot, randomProof);
        ...
        it('Submit invalid process attestation proof should not affect Unirep State', async () => {
            ...
            const userState = await (0, core_1.genUserStateFromContract)(hardhat_1.ethers.provider, unirepContract.address, userIds[storedUserIdx]);
            (0, chai_1.expect)(userState.toJSON()).equal(storedUserState);
            let hashedProof = (0, contracts_1.computeProcessAttestationsProofHash)(randomOutputBlindedUserState, randomOutputBlindedHashChain, randomInputBlindedUserState, randomProof);
            invalidProofIndexes.push(Number(await unirepContract.getProofIndex(hashedProof)));
        });
        ...
        it('Submit attestations to transitioned users', async () => {
            ...
            for (let i = 0; i < transitionedUsers.length; i++) {
                ...
                tx = await unirepContractCalledByAttester.submitAttestation(attestation, epochKey, proofIndex, fromProofIndex, { value: core_1.attestingFee });
                receipt = await tx.wait();
                (0, chai_1.expect)(receipt.status).to.equal(1);
