...
describe('Process attestation circuit', function () {
    ...
    it('successfully process zero attestations', async () => {
        ...
        const initialUserStateTreeRoot =
    ...
    it('Sign up flag should not be overwritten', async () => {
        ...
        for (let i = 1; i < config.numAttestationsPerProof; i++) {
            const attestation: Attestation = new Attestation(
                sameAttesterID,
                BigInt(Math.floor(Math.random() * 100)),
                BigInt(Math.floor(Math.random() * 100)),
    ...
    it('process attestations with wrong attestation record should not work', async () => {
        ...
        let indexWrongAttestationRecord = Math.floor(
            Math.random() * config.numAttestationsPerProof
        )
        ...
        circuitInputs.old_neg_reps[indexWrongAttestationRecord] = Math.floor(
            Math.random() * 100
        ).toString()
        ...
        await throwError(
            circuit,
            circuitInputs,
            'Root mismatch results from wrong attestation record should throw error'
        )
    })
    ...
    it('process attestations with wrong intermediate roots should not work', async () => {
        ...
        await throwError(
            circuit,
            circuitInputs,
            'Root mismatch results from wrong intermediate roots should throw error'
        )
    })
    ...
    it('process attestations with incorrect number of elements should fail', async () => {
        ...
        await throwError(
            circuit,
            circuitInputs,
            'Incorrect number of elements should throw error'
        )
    })
})
