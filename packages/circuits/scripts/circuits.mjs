...
export const circuitContents = {
    ...
    proveReputation: `pragma circom 2.0.0; include "../circuits/proveReputation.circom"; \n\ncomponent main { public [ epoch, attester_id, min_rep, prove_graffiti, graffiti_pre_image ] } = ProveReputation(${GLOBAL_STATE_TREE_DEPTH}, ${EPOCH_TREE_DEPTH}, ${NUM_EPOCH_KEY_NONCE_PER_EPOCH}, 252);`,
    signup: `pragma circom 2.0.0; include "../circuits/signup.circom"; \n\ncomponent main { public [ attester_id, epoch ] } = Signup();`,
    aggregateEpochKeys: `pragma circom 2.0.0; include "../circuits/aggregateEpochKeys.circom"; \n\ncomponent main { public [ start_root, epoch, attester_id, hashchain_index ] } = AggregateEpochKeys(${EPOCH_TREE_DEPTH}, ${AGGREGATE_KEY_COUNT});`,
