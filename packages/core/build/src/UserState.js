...
class UserState {
    constructor(_unirepState, _id, _hasSignedUp, _latestTransitionedEpoch, _latestGSTLeafIndex, _latestUserStateLeaves, _transitionedFromAttestations) {
        ...
        this.genUserStateTransitionProofs = async () => {
            ...
            for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
                ...
                for (let i = 0; i < attestations.length; i++) {
                    ...
                    if (i &&
                        ...
                        hashChainStarter.push(currentHashChain);
                        blindedUserState.push((0, crypto_1.hash5)([
                            this.id.getNullifier(),
                            fromEpochUserStateTree.getRootHash(),
                            BigInt(fromEpoch),
                            BigInt(nonce),
                ...
                const filledAttestationNum = attestations.length
                    ? Math.ceil(attestations.length / this.numAttestationsPerProof) * this.numAttestationsPerProof
                ...
                blindedHashChain.push((0, crypto_1.hash5)([
                    this.id.getNullifier(),
                    currentHashChain,
                    BigInt(fromEpoch),
                    BigInt(nonce),
            ...
            finalUserState.push(fromEpochUserStateTree.getRootHash());
            finalBlindedUserState.push((0, crypto_1.hash5)([
                this.id.getNullifier(),
                finalUserState[0],
                BigInt(fromEpoch),
        ...
        this.genProveReputationProof = async (attesterId, epkNonce, minRep, proveGraffiti, graffitiPreImage, nonceList) => {
            ...
            if (repNullifiersAmount > 0) {
                ...
                (0, assert_1.default)(nonceStarter + repNullifiersAmount <=
                    Number(posRep) - Number(negRep), 'Not enough reputation to spend');
            }
            const circuitInputs = (0, crypto_1.stringifyBigInts)({
                epoch: epoch,
