    ...
    constructor(
        ...
        this.toJSON = (space = 0) => {
            ...
            for (let index in this.epochTreeLeaves) {
                epochTreeLeavesToString[index] = this.epochTreeLeaves[
                    index
                ].map(
                    (l) =>
            ...
            return JSON.stringify(
                {
                    ...
                    GSTLeaves: Object(
                        (0, crypto_1.stringifyBigInts)(this.GSTLeaves)
                    ),
        ...
        this.genEpochTree = async (epoch) => {
            this._checkValidEpoch(epoch)
            const epochTree = await (0, utils_1.genNewSMT)(
                this.setting.epochTreeDepth,
                utils_1.SMT_ONE_LEAF
            )
        ...
        this.epochTreeRootExists = async (_epochTreeRoot, epoch) => {
            ...
            return (
                this.epochTreeRoot[epoch].toString() ==
                _epochTreeRoot.toString()
            )
        }
        /*
        ...
        this.signUp = async (
            ...
            this._checkMaxUser()
            let GSTLeaf
            const USTRoot = await (0, utils_1.computeInitUserStateRoot)(
        ...
        this.epochTransition = async (epoch, blockNumber) => {
            this._checkCurrentEpoch(epoch)
            this._checkBlockNumber(blockNumber)
            this.epochTree[epoch] = await (0, utils_1.genNewSMT)(
                this.setting.epochTreeDepth,
                utils_1.SMT_ONE_LEAF
            )
            ...
            for (let epochKey of this.epochKeyInEpoch[epoch].keys()) {
                ...
                for (
                    ...
                    hashChain = (0, crypto_1.hashLeftRight)(
                ...
                const sealedHashChainResult = (0, crypto_1.hashLeftRight)(
                    BigInt(1),
                    hashChain
                )
                const epochTreeLeaf = {
            ...
            this.globalStateTree[this.currentEpoch] =
                new crypto_1.IncrementalMerkleTree(
        ...
        const emptyUserStateRoot = (0, utils_1.computeEmptyUserStateRoot)(
            this.setting.userStateTreeDepth
        )
        this.defaultGSTLeaf = (0, crypto_1.hashLeftRight)(
            BigInt(0),
            emptyUserStateRoot
        )
        if (_GSTLeaves !== undefined) {
            ...
            for (let key in this.GSTLeaves) {
                this.globalStateTree[key] = new crypto_1.IncrementalMerkleTree(
                    this.setting.globalStateTreeDepth,
                    this.defaultGSTLeaf,
                    2
                )
