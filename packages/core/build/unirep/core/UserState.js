    ...
    constructor(_unirepState, _id, _hasSignedUp, _latestTransitionedEpoch, _latestGSTLeafIndex, _latestUserStateLeaves, _transitionedFromAttestations) {
        ...
        this.toJSON = (space = 0) => {
            ...
            for (let nonce = 0; nonce < this.unirepState.setting.numEpochKeyNoncePerEpoch; nonce++) {
                const epk = (0, utils_1.genEpochKey)(this.id.getNullifier(), epoch, nonce).toString();
        ...
        this.getEpochKeyNullifiers = (epoch) => {
            ...
            for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
                const nullifier = (0, utils_1.genEpochKeyNullifier)(this.id.getNullifier(), epoch, nonce);
        ...
        */
        ...
        this.genVerifyEpochKeyProof = async (epochKeyNonce) => {
            ...
            this._checkEpkNonce(epochKeyNonce);
            const epoch = this.latestTransitionedEpoch;
            const epochKey = (0, utils_1.genEpochKey)(this.id.getNullifier(), epoch, epochKeyNonce, this.unirepState.setting.epochTreeDepth);
        ...
        this._saveAttestations = () => {
            ...
            for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
                const epochKey = (0, utils_1.genEpochKey)(this.id.getNullifier(), fromEpoch, nonce, this.unirepState.setting.epochTreeDepth).toString();
        ...
        this._genNewUserStateAfterTransition = async () => {
            ...
            for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
                const epkNullifier = (0, utils_1.genEpochKeyNullifier)(this.id.getNullifier(), fromEpoch, nonce);
        ...
        this.genUserStateTransitionProofs = async () => {
            ...
            for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
                const epochKey = (0, utils_1.genEpochKey)(this.id.getNullifier(), fromEpoch, nonce, this.unirepState.setting.epochTreeDepth);
                ...
                for (let i = 0; i < attestations.length; i++) {
                    ...
                    if (i && (i % this.numAttestationsPerProof == 0) && (i != this.numAttestationsPerProof - 1)) {
                        ...
                        hashChainStarter.push(currentHashChain);
                        blindedUserState.push((0, crypto_1.hash5)([this.id.getNullifier(), fromEpochUserStateTree.getRootHash(), fromEpoch, nonce]));
                    }
                    const attestation = attestations[i];
                    const attesterId = attestation.attesterId;
                ...
                blindedHashChain.push((0, crypto_1.hash5)([this.id.getNullifier(), currentHashChain, fromEpoch, nonce]));
            ...
            finalUserState.push(fromEpochUserStateTree.getRootHash());
            finalBlindedUserState.push((0, crypto_1.hash5)([this.id.getNullifier(), finalUserState[0], fromEpoch, startEpochKeyNonce]));
            finalBlindedUserState.push((0, crypto_1.hash5)([this.id.getNullifier(), finalUserState[1], fromEpoch, endEpochKeyNonce]));
        ...
        this.genProveReputationProof = async (attesterId, epkNonce, minRep, proveGraffiti, graffitiPreImage, nonceList) => {
            ...
            const epoch = this.latestTransitionedEpoch;
            const epochKey = (0, utils_1.genEpochKey)(this.id.getNullifier(), epoch, epkNonce);
            const rep = this.getRepByAttester(attesterId);
            ...
            if (repNullifiersAmount > 0) {
                ...
                for (let n = 0; n < Number(posRep) - Number(negRep); n++) {
                    const reputationNullifier = (0, utils_1.genReputationNullifier)(this.id.getNullifier(), epoch, n, attesterId);
        ...
        this.genUserSignUpProof = async (attesterId) => {
            ...
            const epochKey = (0, utils_1.genEpochKey)(this.id.getNullifier(), epoch, nonce);
            const rep = this.getRepByAttester(attesterId);
            const posRep = rep.posRep;
            const negRep = rep.negRep;
            const graffiti = rep.graffiti;
