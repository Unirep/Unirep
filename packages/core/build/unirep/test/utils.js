...
const genNewGST = (GSTDepth, USTDepth) => {
    ...
    const GST = new crypto_1.IncrementalMerkleTree(GSTDepth, defaultGSTLeaf, 2);
    return GST;
};
...
const getReputationRecords = (id, unirepState) => {
    ...
    for (let i = 0; i < currentEpoch; i++) {
        for (let j = 0; j < unirepState.setting.numEpochKeyNoncePerEpoch; j++) {
            const epk = (0, core_1.genEpochKey)(id.getNullifier(), i, j);
...
const genEpochKeyCircuitInput = (id, tree, leafIndex, ustRoot, epoch, nonce) => {
    ...
    const epk = (0, core_1.genEpochKey)(id.getNullifier(), epoch, nonce);
    const circuitInputs = {
