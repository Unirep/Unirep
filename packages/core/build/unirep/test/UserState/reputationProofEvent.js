...
describe('Reputation proof events in Unirep User State', function () {
    ...
    describe('Reputation proof event', async () => {
        ...
        it('spend reputation event can attest to other epoch key and update User state', async () => {
            ...
            const userState = await (0, core_1.genUserStateFromContract)(hardhat_1.ethers.provider, unirepContract.address, userIds[otherUserIdx]);
            epoch = Number(await unirepContract.currentEpoch());
            const epkNonce = 0;
            ...
            (0, chai_1.expect)(receipt.status).to.equal(1);
            const userStateAfterAttest = await (0, core_1.genUserStateFromContract)(hardhat_1.ethers.provider, unirepContract.address, userIds[otherUserIdx]);
        ...
        it('invalid reputation proof with from proof index should not update User state', async () => {
            ...
            (0, chai_1.expect)(receipt.status).to.equal(1);
            const userStateAfterAttest = await (0, core_1.genUserStateFromContract)(hardhat_1.ethers.provider, unirepContract.address, userIds[otherUserIdx]);
            (0, chai_1.expect)(userState.toJSON()).equal(userStateAfterAttest.toJSON());
        });
