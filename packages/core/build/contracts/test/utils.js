...
const genProcessAttestationsCircuitInput = async (id, epoch, fromNonce, toNonce, _selectors, _hashChainStarter, _attestations) => {
    ...
    const inputBlindedUserState = (0, crypto_1.hash5)([id.getNullifier(), intermediateUserStateTreeRoots[0], epoch, fromNonce]);
    const circuitInputs = {
        epoch: epoch,
...
const genUserStateTransitionCircuitInput = async (id, epoch) => {
    ...
    intermediateUserStateTreeRoots.push(userStateTree.getRootHash());
    blindedUserState.push((0, crypto_1.hash5)([id.getNullifier(), userStateTree.getRootHash(), BigInt(epoch), BigInt(startEpochKeyNonce)]));
    // Global state tree
    const GSTree = new crypto_1.IncrementalMerkleTree(config_1.circuitGlobalStateTreeDepth, GSTZERO_VALUE, 2);
    ...
    for (let nonce = 0; nonce < config_1.numEpochKeyNoncePerEpoch; nonce++) {
        ...
        const epochKey = genEpochKey(id.getNullifier(), epoch, nonce, config_1.circuitEpochTreeDepth);
        const hashChainResult = (0, crypto_1.genRandomSalt)();
        // Blinded hash chain result
        hashChainResults.push(hashChainResult);
        blindedHashChain.push((0, crypto_1.hash5)([id.getNullifier(), hashChainResult, BigInt(epoch), BigInt(nonce)]));
    ...
    blindedUserState.push((0, crypto_1.hash5)([id.getNullifier(), intermediateUserStateTreeRoot, BigInt(epoch), BigInt(endEpochKeyNonce)]));
    for (let nonce = 0; nonce < config_1.numEpochKeyNoncePerEpoch; nonce++) {
        const epochKey = genEpochKey(id.getNullifier(), epoch, nonce, config_1.circuitEpochTreeDepth);
