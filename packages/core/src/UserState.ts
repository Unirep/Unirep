...
class UserState {
  ...
  constructor(
    ...
    assert(_unirepState !== undefined, "UserState: UnirepState is undefined");
  ...
  public toJSON = (space = 0): string => {
    ...
    for (
      ...
      if (attestations !== undefined)
        transitionedFromAttestationsToString[epk] = attestations.map((a) =>
          a.toJSON()
        );
    }
    return JSON.stringify(
  ...
  public genUserStateTransitionProofs = async () => {
    ...
    for (let nonce = 0; nonce < this.numEpochKeyNoncePerEpoch; nonce++) {
      ...
      for (let i = 0; i < attestations.length; i++) {
        ...
        const USTLeafPathElements = await fromEpochUserStateTree.getMerkleProof(
          attesterId
        );
      ...
      const filledAttestationNum = attestations.length
        ? Math.ceil(attestations.length / this.numAttestationsPerProof) *
  ...
  public genProveReputationProof = async (
    ...
    if (nonceList == undefined)
      nonceList = new Array(this.unirepState.setting.maxReputationBudget).fill(
        BigInt(-1)
      );
    assert(
